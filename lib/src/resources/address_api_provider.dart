...
class AddressApiProvider {
  Future<Object> updateOrCreateAddress(
      DeliveryAddressModel address, CustomerModel customer) async {
      ...
      */
      ...
      if (response.statusCode == 200) {
        ...
        DeliveryAddressModel address =
            DeliveryAddressModel.fromJson(mJsonDecode(response.data)["data"]);
        Map res = Map();
        if (errorCode == 0) {
          res["error"] = errorCode;
          res["address"] = address;
      ...
      */
      ...
      if (response.statusCode == 200) {
        ...
        if (errorCode == 0) {
          ...
          String suburb =
              mJsonDecode(response.data)["data"]["address"]["suburb"];
          var m = Map();
  ...
  Future<List<DeliveryAddressModel>> fetchAddressList(
  ...
  deleteAddress(CustomerModel customer, DeliveryAddressModel address) async {
    ...
    if (await Utils.hasNetwork()) {
      ...
      var response = await dio.post(
          Uri.parse(ServerRoutes.LINK_DELETE_ADRESS).toString(),
