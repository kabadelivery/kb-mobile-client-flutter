  ...
  Widget build(BuildContext context) {
    return Scaffold(
        appBar: AppBar(
            ...
            title: Text("${AppLocalizations.of(context).translate('review')}".toUpperCase())
        ),
        body: SingleChildScrollView(
          child: Column(
              children: (<Widget>[
                // tell us what to do
                SizedBox(height:10),
                Container(margin: EdgeInsets.only(top:40, right: 20, left:20), decoration: BoxDecoration(color: KColors.primaryColor, borderRadius: BorderRadius.all(Radius.circular(5))), padding: EdgeInsets.all(10),
                    child: Text("${AppLocalizations.of(context).translate('please_review_restaurant')}", textAlign: TextAlign.center, style: TextStyle(color: Colors.white))),
                SizedBox(height:10),
...
// message field
                ...
                Container (margin: EdgeInsets.only(left:20, right:20, top:20),decoration: BoxDecoration(borderRadius: BorderRadius.all(Radius.circular(5)), color:Colors.grey.shade200), padding: EdgeInsets.all(10),
                    child: TextField(controller: _reviewTextController, style: TextStyle(color: Colors.black, fontSize: 16) ,maxLines: 5, minLines: 4, textAlign: TextAlign.left,
                  ...
                  children: <Widget>[
                    MaterialButton(padding: EdgeInsets.only(top:15, bottom:15, left:10, right:10), color: Colors.white,
                        child: Row(mainAxisSize: MainAxisSize.min,
                          children: <Widget>[
                            Text("${AppLocalizations.of(context).translate('send_review')} ${widget.rate}/${widget.max_rate}", style: TextStyle(fontSize: 14, color: KColors.primaryColor)),
