                    ...
                    return Column(
                      children: <Widget>[
                        ...
                        Row(mainAxisAlignment: widget.messages[position]?.user_id == 0 ? MainAxisAlignment.end : MainAxisAlignment.start,
                          children: <Widget>[
                            Container(decoration: BoxDecoration(color: widget.messages[position]?.user_id == 0 ?  Colors.white : KColors.primaryColor, borderRadius: BorderRadius.all(Radius.circular(5))),
                              padding: EdgeInsets.all(10),
                              child: Column(
                                children: <Widget>[
                                  Row(mainAxisSize: MainAxisSize.max, mainAxisAlignment: MainAxisAlignment.start,
                                    children: <Widget>[
                                      Text("${widget.messages[position]?.message}", textAlign: TextAlign.left,style: TextStyle(fontSize: 14, color:  widget.messages[position]?.user_id == 0 ? Colors.black : Colors.white),),
                                    ],
                                  ),
                                  SizedBox(height: 10),
                                  Row(mainAxisSize: MainAxisSize.max, mainAxisAlignment: MainAxisAlignment.end, children: <Widget>[
                                    Text("${Utils.readTimestamp(widget.messages[position]?.created_at)}",  textAlign: TextAlign.right, style: TextStyle(fontSize: 11, color:  widget.messages[position]?.user_id == 0 ? Colors.black : Colors.white),),
                                  ])
                                ],
                              ),
                            ),
              ...
              child: Row(mainAxisSize: MainAxisSize.max,
                children: <Widget>[
                  ...
                  SizedBox(height: 50, width: MediaQuery.of(context).size.width-80,
                    child: Container(padding: EdgeInsets.only(left:8, right:8),child: TextField(controller: _messageController, decoration: InputDecoration.collapsed(hintText: "Insert your message")), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.all(Radius.circular(4))),),
                  ),
                  SizedBox(width: 5),
